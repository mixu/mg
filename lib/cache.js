...
exports.get = function get(name, id, onDone) {
  ...
  if(item) {
    console.log('Fulfilling .get() from local cache', name, id);
...
exports.store = function(name, values) {
  ...
  (Array.isArray(values) ? values : [ values ]).forEach(function(value) {
    if(value && value.id) {
      console.log('Storing fetch result in cache', name, value.id);
      cache[name][value.id] = value;
    } else {
      console.log('Unknown cache.store value', value);
    }
  });
};
