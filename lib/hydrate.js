...
Hydration.prototype.link = function(name, id) {
  var self = this;
        ...
        (Array.isArray(ids) ? ids : [ ids ]).forEach(function(modelId) {
          ...
          switch(typeof modelId) {
            ...
            case 'string':
              ...
              util.set(self.cache[name][id], key, self.cache[modelName][modelId]);
              break;
            case 'object':
              // model.id
              if(modelId.id) {
                log.info('Link from', name, id, 'to', modelName, modelId.id);
                util.set(self.cache[name][id], key, self.cache[modelName][modelId.id]);
              }
          }
        });
      });
...
Hydration.prototype.hydrate = function(name, model, done) {
  var self = this;
  if(!model.id) {
    log.warn('Skipping hydration, no model.id', model);
    return done();
  }
  ...
  function fetch() {
    if(self.queue.length > 0) {
      ...
      done(null, self.link(name, model.id));
    }
  }
};
