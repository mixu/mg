...
Hydration.prototype.linkRel = function(name, instance) {
  ...
  Object.keys(rels).forEach(function(key) {
    ...
    // no tasks if ids is not set
    if(!ids) {
      log.info('Unlinkable value', name, instance.id, key, ids);
      return;
    }
...
Hydration.prototype.flatten = function(name, model) {
  ...
  Object.keys(rels).forEach(function(key) {
    ...
    (Array.isArray(ids) ? ids : [ ids ]).forEach(function(model) {
      // items may be either numbers, strings or models
      switch(typeof model) {
        case 'object':
          // model.id
          if(model.id) {
            if(!self.inputCache[modelName]) {
              self.inputCache[modelName] = {};
            }
            self.inputCache[modelName][model.id] = model;
          }
      }
    });
