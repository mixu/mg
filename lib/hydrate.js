...
module.exports = function hydrate(name, model, rels, onDone) {
  var self = this,
      ...
      // input cache
      inputCache = {},
      queue = new Queue(function(name, id, onDone) {
        cache.get(name, id, onDone);
      });
  ...
  function filter(modelClass, id, parentClass, pId) {
    var isMatch = true,
        local = cache.local(modelClass, id);
    // check if this a rel we want

    // add each dependency into the queue
    if(isMatch) {
      if(local && queue.canQueue(modelClass, id)) {
        queue.seen(modelClass, id);
        // merge, store and iterate
        afterFetch(modelClass, id,
          merge(modelClass, id, inputCache[modelClass] && inputCache[modelClass][id], local, false)
        );
      } else {
        log.info('Queue hydration for:', modelClass, id);
  ...
  queue.on('fetched', function(modelClass, id, model) {
    // merge, store and iterate
    afterFetch(modelClass, id,
      merge(modelClass, id, inputCache[name] && inputCache[name][id], model, true)
    );
  });

  function afterFetch(modelClass, id, model) {
